...
def createModel(trainX, trainBox, trainConf, valX, valBox, valConf):
    ...
    layer_3 = layers.Conv2D(64, kernel_size=1, padding="same", strides=2, activation='sigmoid')(layer_2)
    pool_layer = layers.MaxPooling2D(pool_size=(3, 3), strides=(1, 1), padding="same")(layer_3)
    layer_4 = layers.Conv2D(16, kernel_size=1, padding="same", activation='sigmoid')(pool_layer)
    layer_5 = layers.Conv2D(32, kernel_size=3, padding="same", activation='sigmoid')(layer_4)

    final1 = layers.Conv2D(16, kernel_size=3, padding="same", strides=2, activation='relu')(layer_5)
    final2 = layers.Conv2D(16, kernel_size=3, padding="same", strides=2, activation='relu')(layer_5)
    output_1 = layers.Conv2D(4, kernel_size=4, padding="same", strides=2, activation='relu')(
        final1)  # bounding box, first value is ignored so loss works
    ...
    model.compile(optimizer='Adadelta', loss=[customLoss, customLoss2])
    print(model.summary())
    ...
    out = model.fit(x=trainX, y=[trainBox, trainConf], validation_data=[valX, [valBoxCompound, valConf]], epochs=100)
